...
export function explicitlyRequestsSearch(text: string): boolean {
  ...
  return clauses.some(clause => {
    if (declinesWebSearch(clause)) return false;
...
/** Respect a user opt-out before automatically requesting artwork research. */
export function declinesWebSearch(text: string): boolean {
  return /\b(?:do not|don't|without|no need to)\s+(?:web\s+)?(?:search|google|browse)\b|(?:не\s+(?:надо\s+|нужно\s+)?|без\s+)(?:гугл|гугли|ищи|искать|поиск)/u.test(text.toLowerCase());
}
