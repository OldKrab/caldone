...
This document describes the production Android app in `mobile/`, with package `dev.caldone.app` and scheme `caldone`. [Product scope](product.md) owns behavior; [migration guidance](migrations.md) describes the separate installation introduced in 1.2.0.

Implementation references are `mobile/src/design/tokens.ts`, shared components, and feature screens. The frontmatter records native values: layout dimensions are dp and text sizes are sp, represented by React Native numeric styles. These are not CSS requirements. Typography entries describe observed roles rather than a complete centralized type scale.

The palette is static and light. Do not infer dark-theme support, Dynamic Color, tablet readiness, or full accessibility compliance from these source values.
...
- **Brand artwork:** `mobile/assets/caldone-fork-icon.png` is the white bent-fork/check silhouette on an emerald square, shared by the launcher, welcome brand row, and About. The selected 1254×1254 PNG is opaque RGB; generation provenance is in the sibling `.md` file. The Android launcher plugin applies 12% foreground insets for adaptive masks. No monochrome icon is configured. Asset inspection does not substitute for launcher-mask validation on a device.
...
- **Assistant:** Restrained conversation, readable prose, quiet operational states and a reachable composer. Messages and clarification questions use native long-press text selection; no persistent Copy buttons. Capture, inline answers and chat share one conversation and one visible work state. Hosted web searches appear in the same chronological action group as local tools, and completed searches remain history when reopening chat. A persisted form answer replaces its form with progress during processing; failure makes unresolved questions available again. Reanalysis is visible on the diary card and meal detail while the previous estimate stays readable. Meal clarifications and structured Assistant questions offer selectable answers, Not sure, and optional custom text. No answer is preselected; Send answers submits the explicit selections together. Choices wrap into 48dp-minimum touch targets, and questions remain active until their IDs are explicitly answered or dismissed. Unrelated messages leave them open; forms may submit a partial answer. Before the first estimate, questions use the same keyboard-aware scrolling layout as the meal detail form. Legacy meal questions remain readable without generated presets. Transient connection failures retry once after foreground recovery; persistent failures offer an explicit retry. Provider HTML/SVG failures show a localized summary. Show details opens selectable literal diagnostic text in AppDialog, with Copy details and Close; recognized credential fields are redacted from display and copying. Apply this to saved errors as well as live chat, selected-meal, and tool failures. One tool call stays a direct operational row; consecutive calls across internal messages share a stable collapsible group. User messages and assistant prose break groups to preserve chronology. Labels stay generic until execution starts; execution events supply running, completed, failed, and cancelled states. Active and failed rows remain visible when completed details collapse. Action receipts expose before/after snapshots through “What changed” and keep Undo separate; missing historical detail is stated explicitly. Avoid AI decoration, fake progress, or visual claims of nutrition certainty.
- **App updates:** Keep the Settings update page flat and scrollable, with installed version and check status before release details, a primary Download and install or Continue action, and quiet progress, cancellation, and manual-check controls. The release `AppDialog` leads with a concrete user benefit, followed by version, size, and short highlights; Full release notes and Later remain separate actions. Operational copy follows the app language; authored release copy stays in English.

### Menus and dialogs

Use `AnchoredMenu` for three-dot and header overflow menus, beside their trigger without a dimmed backdrop. Use `AppDialog` for app-owned confirmations, notices, errors, and bottom actions without an anchored trigger. Its title and message scroll independently of the action rows so long release notes fit while actions remain reachable. App-owned interactions must not use `Alert.alert`, stock Android action dialogs, or other default platform UI.
