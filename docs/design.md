...
- **App updates:** Keep the Settings update page flat and scrollable, with installed version and check status before release details, a primary Download and install/Continue action, and quiet progress, cancellation, and manual-check controls. The release `AppDialog` leads with a concrete user benefit, followed by version, size, and highlights; Full release notes and Later remain separate actions. Operational copy follows the app language; authored release copy stays in English. This describes the source; screenshots and real-device update behavior remain unverified.
- **Menus and dialogs:** Use `AnchoredMenu` beside overflow triggers without a dimmed backdrop. Use `AppDialog` for app-owned confirmations, notices, errors, and unanchored actions. Its title and message scroll independently of the action rows so long release notes can fit while preserving reachable actions; verify this on-device with font scaling. OS-owned permission, authentication, camera, photo-picker, and share interfaces may remain native.
