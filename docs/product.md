...
The current source implements the agreed update flow in [#51](https://github.com/OldKrab/caldone/issues/51). Typechecks and native builds pass; UI rendering and a real upgrade with data preservation remain unverified because the emulator's boot/system services blocked device checks.

- Automatic update checks are enabled by default and run on opening or returning to the app, at most once per eight hours. Settings also offers a manual check.
- Discovering an update does not download it. Every download requires an explicit **Download and install** action; there is no automatic-download or unattended-installation setting. Once ready, open Android's installation confirmation without an additional app confirmation, provided no user work is active.
- Offer each new stable release once in a dismissible `AppDialog` when Today is idle, without interrupting active work. Show its version, download size, release highlights, and Download and install/Full release notes/Later actions. After dismissal, the update remains available in Settings.
