...
test('submission hides the question immediately, retains the answer and restores questions on failure', async () => {
  ...
  try {
    assert.deepEqual(render().map(item=>item.kind),['message','progress']);
    assert.equal((render()[0] as any).message.text,'All of it');
    fail(Error('offline'));
...
test('after submission succeeds only remaining questions return', async () => {
  ...
  assert.deepEqual((rendered[0] as any).message.questions,['Which sauce?']);
