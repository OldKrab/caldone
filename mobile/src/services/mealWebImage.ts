...
export async function parseMealResult(
  ...
  const webImage = await resolveMealWebImage(raw.webImageSourceUrl, result.research, hasUserPhotos, fetchPage);
  return webImage ? { ...analysis, webImage } : analysis;
}

/** Only a provider-observed source page can supply optional illustrative art. */
export async function resolveMealWebImage(
  ...
  if (hasUserPhotos || research?.status !== 'completed') return undefined;
  const sourceUrl = publicWebUrl(candidate);
  if (!sourceUrl || !research.sources.some(source => publicWebUrl(source.url) === sourceUrl)) return undefined;
  try {
    const fetch = fetchPage ?? (await import('expo/fetch')).fetch as typeof globalThis.fetch;
    return await fetchPreviewImage(sourceUrl, fetch);
